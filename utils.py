...
def load_train_data(image_path, load_size=286, fine_size=256, is_testing=False):
    ...
    mask_A = imread(image_path[0].replace('trainA','maskTrainA').replace('testA','maskTestA').replace('.jpg','_mask.png'), is_grayscale=True)
    ...
    img_A = img_A/127.5 - 1.
    mask_A = mask_A/127.5 - 1.
    img_B = img_B/127.5 - 1.

    mask_A = np.expand_dims(mask_A, axis=2)
    img_AB = np.concatenate((img_A, mask_A, img_B), axis=2)
